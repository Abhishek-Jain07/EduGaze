    ...
    def _match_names(self, faces: Dict[int, BBox], ocr_boxes: list[OCRBox]):
        ...
        for sid, bbox in faces.items():
            ...
            for ob in ocr_boxes:
                ...
                # Only consider boxes below the face
                if oy < face_bottom_y - 10:  # Allow small overlap
                    continue
                ...
                # Bonus for longer names (full names like "Abhishek Jain" vs just "Jain")
            ...
            if best_name is None and ocr_boxes:
                ...
                for ob in ocr_boxes:
                    ox, oy, ow, oh = ob.box
                    # Check if OCR box overlaps horizontally with face
                    if (ox <= face_left + w and ox + ow >= face_left and 
                        oy >= face_bottom_y - 50 and ob.confidence >= 25):
                        best_name = ob.text
                        best_conf = ob.confidence
                        best_box = ob
                ...
                # If still no name, and only one face, use best OCR result
                if best_name is None and len(faces) == 1:
                    # Prefer names with more words (full names over last names)
                    candidates = [ob for ob in ocr_boxes if ob.confidence >= 25]  # Lower threshold to 25%
                    if candidates:
                        # Sort by word count first, then confidence
                        fallback = max(candidates, key=lambda b: (len(b.text.split()), b.confidence))
                        if fallback:
            ...
            if sid not in self.students:
                ...
                if best_name and len(best_name) >= 2 and best_conf >= 25:  # Lower threshold to 25% to catch more names
                    ...
                    # Count words to prefer full names ("Abhishek Jain" > "Jain")
                    current_words = len(s.name.split()) if s.name else 0
                    best_words = len(best_name.split()) if best_name else 0
                    
                    is_better = (
                        s.name.startswith("Unknown") or  # Always update Unknown
                    ...
                    # Even if confidence is slightly low, always update Unknown names
                    if s.name.startswith("Unknown") and best_conf >= 25:
                        old_name = s.name
                        s.name = best_name
                        s.ocr_confidence = best_conf
                        print(f"[Pipeline] ✓ Updated Unknown student {sid} to '{best_name}' (conf={best_conf:.1f}%)")
                    else:
                        print(f"[Pipeline] Keeping '{s.name}' over '{best_name}' (words: {len(s.name.split()) if s.name else 0} vs {len(best_name.split())}, conf: {s.ocr_confidence:.1f} vs {best_conf:.1f})")
                s.last_bbox = bbox
