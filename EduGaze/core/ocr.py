...
class ZoomNameOCR:
        ...
        for box in deduplicated:
            ...
            if extracted_names:
                ...
                if self._is_valid_name(text):
                    ...
                    print(f"[OCR] ✗ Rejected garbage text: '{text}'")
        
        # Try to combine nearby boxes that might form full names
        # This is a final pass to merge names detected separately (e.g., "Abhishek" and "Jain")
    ...
    def _combine_nearby_words(self, boxes: List[OCRBox]) -> List[OCRBox]:
        ...
        Zoom names can be on one line (Abhishek Jain) or stacked vertically.
        """
        if not boxes:
            return []
        ...
        while i < len(sorted_boxes):
            ...
            while j < len(sorted_boxes):
                ...
                # This helps catch "Abhishek" and "Jain" even if they're separated
