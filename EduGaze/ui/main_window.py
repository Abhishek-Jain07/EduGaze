...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        session_hint = QtWidgets.QLabel("EduGaze now captures the entire primary monitor for maximum reliability.")
        session_hint.setWordWrap(True)
        session_hint.setObjectName("HintLabel")
        controls_layout.addWidget(session_hint, 2, 0, 1, 3)
    ...
    def start_capture(self):
        ...
        if not log_path:
            QtWidgets.QMessageBox.warning(self, "Invalid path", "Please provide a valid log path.")
            return

        # Ensure region is cleared (full screen capture)
        self.pipeline.capturer.set_region(None)
